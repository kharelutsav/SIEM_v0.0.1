...
impl Normalizer {
    ...
    pub fn _json_parser (&self, raw_log: &JsonValue, str_log: &mut JsonValue, key_: Option<&str>) {
        // JSON Format //
        ...
        let key_ = key_.unwrap_or("");
        for (key, value) in raw_log.entries() {
            if value.is_empty() {
                continue;
            }
            if value.is_object() {
                self._json_parser(&value, str_log, Some(&format!("{key_}{key}_")));
            }
                ...
                if value[0].is_object() {
                    for index in 0..value.len() {
                        ...
                        self._json_parser(obj, str_log, Some(&format!("{key_}{key}_{index}_")));
                    }
                }
                else {
